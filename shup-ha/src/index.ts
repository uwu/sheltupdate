...
async function reportNodeHealth(up: boolean, env: Env, envName: string, origins: Origin[], origin: Origin) {
	...
	if (lastIncident && lastIncident.allNodes.split(";").includes(origin.url)) {
		...
		if (up === weWerePreviouslyUp) return;
	}
	...
	for (const node of lastNodesUpSet) if (!allNodesSet.has(node)) lastNodesUpSet.delete(node);

	if (lastNodesUpSet.has(origin.url) !== up) {
		if (up) lastNodesUpSet.add(origin.url);
		else lastNodesUpSet.delete(origin.url);
	}
...
`;
		...
		for (const o of origins) {
			...
			console.log("request failed,", considerNodeFailed ? "rolling over" : "returning error", url.hostname, o.name, {
				status: resp?.status,
				headers: resp && Object.fromEntries(resp.headers.entries()),
			});

			if (considerNodeFailed) await reportNodeHealth(false, env, url.hostname, origins, o);
	...
	async scheduled(controller, env, ctx) {
		...
		for (const environment in CONFIG)
			for (const origin of CONFIG[environment]) await checkAndReportHealth(env, environment, CONFIG, origin);
	},
